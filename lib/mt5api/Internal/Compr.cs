...
namespace mtapi.mt5
{
    class Compr
    {
        ...
        unsafe uint DMUL(uint a, uint b)
        {
            return ((uint)((a) * (b)));
        }
        ...
        readonly uint M4_MAX_OFFSET = 0xbfff;

        unsafe bool LZO_CHECK_MPOS_NON_DET(ref byte* m_pos, uint m_off, byte* inn, byte* ip, uint max_offset)
        {
            m_pos = ip - (uint)PTR_DIFF(ip, m_pos);
            return PTR_LT(m_pos, inn) ||
        ...
        readonly int M2_MAX_OFFSET = 0x0800;
        ...
        readonly uint M3_MAX_OFFSET = 0x4000;
        readonly byte M3_MARKER = 32;
        readonly byte M4_MARKER = 16;
        readonly byte M3_MAX_LEN		=33;
        readonly byte M4_MAX_LEN		=9;
        ...
        unsafe uint DoDeflate(byte* pSrc, uint szSrc, byte* pDst, uint szDst, byte*[] m_pBuffer)
        {
            ...
            for (; ; )
            {
                ...
                if (PS(ref m_pos, ref ip) || PS(ref m_pos, ref ip)
                    || PS(ref m_pos, ref ip) || PS(ref m_pos, ref ip)
                    || PS(ref m_pos, ref ip) || PS(ref m_pos, ref ip))
                {
                    --ip;
                    uint m_len = Pd(ip, ii);
